...
class Action
{
	...
	function save_survey()
	{
		...
		foreach ($_POST as $k => $v) {
			if (!in_array($k, array('id', 'user_id')) && !is_numeric($k)) {
				if ($k === 'type') {
					$v = intval($v); // Convert 'type' value to integer
				}
				if (empty($data)) {
					$data .= " $k='$v' ";
				} else {
		...
		// Insert or update survey_set table
		if (empty($id)) {
			$save_survey_set = $this->db->query("INSERT INTO survey_set SET $data");
			$survey_id = $this->db->insert_id; // Get the last inserted survey ID
		} else {
			$save_survey_set = $this->db->query("UPDATE survey_set SET $data WHERE id = $id");
			$survey_id = $id; // Use the existing survey ID
		}

		// Delete existing survey-user relationships
		$this->db->query("DELETE FROM survey_user WHERE survey_id = $survey_id");

		// Insert new survey-user relationships
		if (isset($user_id) && is_array($user_id)) {
			foreach ($user_id as $uid) {
				$this->db->query("INSERT INTO survey_user (survey_id, user_id) VALUES ($survey_id, $uid)");
			}
		}
	...
	function save_answer()
	{
		...
		foreach ($qid as $k => $v) {
			...
			$data .= ", tahun='" . $_POST['tahun'] . "' "; // Tambahkan baris ini untuk menyimpan tahun
		...
		if (isset($save) && !in_array(false, $save, true)) { // Periksa apakah semua insert berhasil
			return 1; // Berhasil disimpan
		} else {
